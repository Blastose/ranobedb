	...
	for (const series of params.series) {
		...
		await params.trx
			...
			.values({
				...
				website: series_to_update.website,
	...
	for (const id of params.series_ids) {
		...
		await params.trx
			...
			.values({
				...
				website: series_to_remove.website,
		...
		await this.ranobeDB.db.transaction().execute(async (trx) => {
			...
			await trx
				...
				.set({
					...
					website: data.series.website ?? null,
