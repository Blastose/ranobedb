...
import { defaultUserListLabelsArray, defaultUserListLabelsMap, languagesArray, releaseFormatArray } from '$lib/db/dbConsts';

const maxNumberValue = 2147483647;

const apiUserListSeriesSchema = userListSeriesSchema.extend({
  seriesId: z.number(),
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const invalidLabelIds = data.selectedCustLabels.filter(
    (id) => !userCustLabelIds.has(id),
  );

  if (invalidLabelIds.length > 0) {
    return error(
      400,
      `Invalid or non-existent custom label ID(s): ${invalidLabelIds.join(', ')}`,
    );
  }
