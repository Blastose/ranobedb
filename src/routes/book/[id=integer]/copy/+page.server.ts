...
async function getBookAndFormForEditing(ds: { params: RouteParams; locals: App.Locals; url: URL }) {
	const { locals, url, params } = ds;
	if (!locals.user) {
...
export const load = async ({ params, locals, url }) => {
	const { book, form } = await getBookAndFormForEditing({ params, locals, url });
	return { book, form };
};
